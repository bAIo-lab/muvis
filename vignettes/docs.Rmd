...
plt.assoc(data, vars = c("LBXTC", "LBXVIE"))
...
plt.assoc(data, vars = c("PAD600"), levels = 15)
```

Density plot for "RIDAGEYR" (age):
```{r plot2, echo=TRUE, message=FALSE, warning=FALSE}
plt.assoc(data, vars = c("RIDAGEYR"), levels = 15)
```
...
plt.assoc(data, vars = c("LBXTHG", "DRD360"), levels = 15)
```
...
plt.assoc(data, vars = c("LBXHGB", "LBXSIR"), levels = 15)
```
