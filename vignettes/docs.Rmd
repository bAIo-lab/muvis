...
```{r preprocess}
data <- data.preproc(Nhanes, levels = 15)
...
## mini.forest
Employ `mini.forest` with BIC to construct a mixed-interaction model fitting the data.
```{r mf, echo=TRUE, message=FALSE, warning=FALSE}
mf <- mini.forest(data, stat = "BIC", community = T, plot = F)
mf$network
mf$betweenness
```
...
mf.val <- graph.vis(mf$graph, plot = T, plot.community = T)
mf.val$communities
```
## violating.vars
...
plt.assoc(data, vars = c("LBXTC", "LBXVIE"), levels = 15, interactive = T)
violating.vars(data[75:160], var1 = data$LBXTC, var2 = data$LBXVIE, permute = 100, levels = 15) -> KL
...
## plt.assoc
Use `plt.assoc` for different visualizations. 
...
plt.assoc(data, vars = c("PAD600"), levels = 15, interactive = F)
```

Density plot for "RIDAGEYR" (age):
```{r plot2, echo=TRUE, message=FALSE, warning=FALSE}
plt.assoc(data, vars = c("RIDAGEYR"), levels = 15, interactive = F)
```
...
plt.assoc(data, vars = c("LBXTHG", "DRD360"), levels = 15, interactive = F)
```
...
<!-- plt.assoc(data, vars = c("DIQ010", "PAD590"), levels = 15) -->
<!-- ``` -->
...
edges[edges$from == "LBXHGB" | edges$to == "LBXHGB", ]
plt.assoc(data, vars = c("LBXHGB", "LBXSIR"), levels = 15, interactive = F)
```
...
<!-- plt.assoc(data, vars = c("PAD590"), levels = 15, pie = TRUE) -->
<!-- ``` -->
