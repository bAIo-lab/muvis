...
## data.preproc
The dataset has some missing values. We'll use `data.preproc` to impute missing values and to specify categorical and continuous variables (Setting level = 15). The first 63 columns are continuous.
```{r preprocess}
...
ggm(data[,1:63], significance = 0.05, rho = 0.1, community = TRUE, methods = c("glasso", "sin"), plot = F) -> g
g$network
g$betweenness
```
...
div2(data[64:dim(data)[2]], var1 = data$LBXTC, var2 = data$LBXVIE, permute = 1000, levels = 15) -> KL
...
plot(data, vars = c("LBXTHG", "DRD360"), levels = 15)
```
...
plot(data, vars = c("PAD590"), levels = 15, pie = TRUE)
```
